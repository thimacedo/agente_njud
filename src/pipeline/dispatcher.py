...
class _EstadoArquivoUnico:
    ...
    def __init__(self, caminho: Path, arquivo: str, njud: str):
        from pipeline.single_process import EstadoArquivo
...
def executar(pasta_boletins: Path, pasta_saida: Path, max_workers: int | None,
    ...
    if pendentes:
        ...
        # CORREÇÃO: criar NOVA fila — a fila antiga foi join()'ada e seus
        # workers encerrados; reusá-la faria as tarefas penderem para sempre.
        fila = mp.Queue()
        processos = []
        for i in range(n_workers):
            p = mp.Process(target=worker_loop, args=(i, fila, pasta_estado, threads_por_worker, usar_whisper_timestamped))
            p.start()
            processos.append(p)
        # REPOE as tarefas pendentes na nova fila
