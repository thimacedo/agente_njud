...
logger = LogPipeline(settings.DIR_OUTPUT / "_logs")

PASTA_DESTINO_DRIVE_PAI = Path(settings.DIR_DRIVE_JORNAIS)
...
ORIGEM = settings.BASE_DIR / "JORNAIS"
DESTINO_CORTES = Path(settings.BOLETINS_CORTADOS)
DESTINO_SAIDA = Path(settings.DIR_OUTPUT)
...
def executar_pipeline_autonomo():
    ...
    modelo = WhisperModel(njud_settings.MODELO_WHISPER, device="cpu", compute_type=njud_settings.COMPUTE_TYPE, cpu_threads=2)
    ...
    with open(settings.DIR_OUTPUT / "relatorio_integridade_autonomo.json", "w", encoding="utf-8") as f:
    ...
    if copias_falha:
        pendentes_path = settings.DIR_OUTPUT / "pendentes_drive.json"
