...
from src.gravador_inteligente.edicao_boletins import (
    EdicaoConfig,
    OperacaoDeEdicao,
    detectar_gatilhos,
    detectar_repeticoes,
...
def test_limita_por_janela_temporal():
    ...
    textos_diferentes = [
        ...
        "O futebol brasileiro é ótimo",
        "A culinária regional é rica",
        "O transporte público melhorou",
        "A educação precisa de investimentos",
        "O hospital foi reformado",
        "A praça ganhou nova iluminação",
        "O trânsito está mais organizado",
        "A biblioteca foi reaberta",
    ]
    for i, t in enumerate(range(4000, 24000, 2000)):
        frases.append(
            {
                "texto": textos_diferentes[i % len(textos_diferentes)],
                "inicio": t,
                "fim": t + 1000,
                "palavras": [],
            }
...
def test_config_default():
    ...
    assert cfg.limiar_similaridade == 0.7
