...
from src.gravador_inteligente.montagem_boletins import (
    AssetsConfig,
    MontagemConfig,
...
def test_separar_cabeca_off_com_pausa():
    ...
    audio = _criar_audio_silencioso(10000)
    pausa_info = {"pausa_ms": 3000, "cabeca_inicio_ms": 0, "tem_claquete": False}
    cabeca, off = separar_cabeca_off(audio, pausa_info=pausa_info)
...
def test_separar_cabeca_off_sem_pausa():
    ...
    cabeca, off = separar_cabeca_off(audio, pausa_info=None)
    assert len(cabeca) == 3000
    assert len(off) == 6000


def test_separar_cabeca_off_auto_detecta():
    """separar_cabeca_off com pausa_info=None detecta automaticamente."""
    audio = _criar_audio_silencioso(12000)
    mock_pausa = {"pausa_ms": 4000, "cabeca_inicio_ms": 0, "tem_claquete": False}
    with patch("src.gravador_inteligente.montagem_boletins.detectar_pausa_principal", return_value=mock_pausa):
        cabeca, off = separar_cabeca_off(audio, pausa_info=None)
...
def test_montagem_config_defaults():
    ...
    assert cfg.pausa_threshold_ms == 500
...
def test_auditar_boletim_com_arquivo_real(real_test_audio, tmp_path, real_assets):
    ...
    from src.gravador_inteligente.montagem_boletins import auditar_boletim
